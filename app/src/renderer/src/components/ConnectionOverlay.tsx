...
export const ConnectionOverlay: React.FC<ConnectionOverlayProps> = (props) => {
  const { ports, selected, onSelectPort, onConnect, error, isConnected } = props
  ...
  // Trigger animation out when connected
  useEffect(() => {
    if (isConnected) {
  ...
  // Don't render if connected and animation is complete
  if (isConnected && isAnimatingOut) {
    // Keep the overlay for the animation duration
    setTimeout(() => {}, 1000)
  }
  ...
  return (
    <div
      className={cn(
        'absolute inset-0 z-40 flex items-center justify-center transition-all duration-700 ease-out',
        isAnimatingOut && 'pointer-events-none opacity-0'
      )}
      style={{
        background: isAnimatingOut
          ? 'radial-gradient(circle at center, rgba(0,0,0,0) 0%, rgba(0,0,0,0) 100%)'
    ...
    >
      <Card
        className={cn(
          'mx-4 w-full max-w-lg border border-white/10 shadow-2xl transition-all duration-700 ease-out',
          isAnimatingOut && 'translate-y-12 scale-90 opacity-0'
        )}
        style={{
              ...
              <g>
                <g>
                  <g>
                    ...
                    <path
                      className="cls-12"
                      d="M162.43,79.32c-4.2,0-7.33-1.27-8.61-3.48-1.26-2.19-.81-5.51,1.24-9.10,2.44-4.28,6.9-8.5,12.24-11.58,5.37-3.1,11.07-4.81,16.05-4.81,4.2,0,7.33,1.27,8.61,3.48,1.26,2.19.81,5.51-1.24,9.10-2.44,4.28-6.9,8.5-12.24,11.58-5.38,3.1-11.07,4.81-16.05,4.81Z"
                    />
                  </g>
                  <g>
