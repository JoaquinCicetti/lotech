...
export const EmergencyStopOverlay: React.FC = () => {
  ...
  return (
    <div
      className="fixed inset-0 z-[200] flex items-center justify-center bg-black/30 backdrop-blur-[2px] pointer-events-auto"
      onClick={handleDismiss}
    >
      <div
        className="rounded-lg border-2 border-red-600 bg-red-950/95 p-8 shadow-2xl relative animate-in fade-in zoom-in duration-200"
      ...
      >
        <Button
          ...
          variant="ghost"
          onClick={handleDismiss}
          className="absolute top-2 right-2 text-red-400 hover:text-red-300 hover:bg-red-900/50"
        >
          <X className="h-4 w-4" />
        </Button>
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-xs text-red-400/70 mt-2">Click aquí o fuera para ocultar</p>
        </div>
      </div>
    </div>
  )
